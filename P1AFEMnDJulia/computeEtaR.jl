function computeEtaR(x,coordinates,elements,element2neighbour,f,g,volumes,
    grad)
    ...
    # vertex numbers of the simplices, and element2neighbour that contains in
    # each row the neighbour numbers of an element. The approximated solution
    # is expected as a vector x. The function handles f and g define the right
    ...
    G = zeros(nE,nD)
    for k = 1:nD+1
        G .+= grad[k] .* x[elements[:,k],ones(Int64,nD)]
    end
    # Compute du_h/dn for each element and hyperface
    dudn = zeros(nE,nD+1)
    bary,wg = quadnd(nD-1,5)
    facets = vcat(collect(combinations(1:nD+1,nD))[end:-1:1]'...)
    for k = 1:nD+1
         J = zeros(nE,nD)
         ...
         J[idx,:] = G[idx,:]
         dudn[:,k] = -sum(grad[k].*J,dims=2) ./ sqrt.(sum(grad[k].^2,dims=2))
    ...
    fint = zeros(nE,1)
    for k = 1:size(bary,1)
        fint .+= f(reshape((bary[k,:]'*C)',:,nD)).^2 * wg[k]
    end
    # Determine residual based error estimator
    etaR = volumes.*((factorial(nD)*volumes).^(2/nD).*fint+nD*sum(dudn.^2,dims=2))
    return etaR
end
