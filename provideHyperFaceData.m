...
% Calculation of additional mesh data. Especially, hyperface data is
% collected.
%
...
% in hyerface 2 elements. The hyperfaces of the simplices are assembled in
...
%*** ascending sorting of nodes of all faces 
faces = sort(reshape(elements(:,ordering),[],nN-1),2);
%*** add boundary faces 
for j = 1:nB
    boundary = varargin{j};
    if ~isempty(boundary)
        faces(ptr(j)+1:ptr(j+1),:) = sort(boundary,2);
    end
end
%*** create unique face numbering, element2hyperfaces, and boundary2hyperfaces
[hyperface2nodes,~,J] = unique(faces,'rows');
