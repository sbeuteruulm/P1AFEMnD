...
% Extract boundary data from an FEM mesh with given information of elements
...
% by their vertex numbers row wise. The information is extracted of the
...
%   is given and can be used for the approximation of a soution.
%
%Remark:
...
nE = size(elements,1);
nN = size(elements,2);
faces = flipud(nchoosek(1:nN,nN-1)-1);
varargout = cell(nB,1);
for k = 1:nB
  [i,j,~] = find(element2neighbour == -k);
  idx = i * ones(1,nN-1) + faces(j,:) * nE;
